...
class Integrity(rdt):
    ...
    def isACK0(self, rcvpkt :bytes, seqNum :int):
        (msg_type, recv_seq_num, _, _), _ = Msg_functions.__unpack_helper(rcvpkt)
    ...
    def notcorrupt(self ,rcvpkt :bytes):
        (msg_type, seq_num, recv_checksum, payload_len), payload = Msg_functions.__unpack_helper(rcvpkt)
        init_msg = struct.Struct('B?HH').pack(msg_type, seq_num, 0, socket.htons(payload_len)) + payload
        calc_checksum = Checksum.calc_checksum(bytearray(init_msg)) #precisa chamar corretamente, tá errado isso aq
